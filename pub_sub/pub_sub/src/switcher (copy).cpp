...
{
	printf("\nturnrobot::\tSetting Flag to 1\tturning to %f angle...\t", s);
	flag = 1;
...
void switcherCallback(const geometry_msgs::Point::ConstPtr& msg)
{
	if(msg->x==0.0f){
		...
		f=1;
		t=(int)msg->x;
		float tt = (2.35/360)*msg->y;
		if(flag==0)
		{
			
			printf("callback::\tFlag is 0... turning to %f angle...\n", tt);
			turnRobot(tt);
		}
		else
		{
			printf("callback::\tFlag is 1... not turning..\n");
		}
	}
}
